<?php 
	...
	class alumnoModelo extends mainModel{
		...
		$sql=$conn->prepare("INSERT INTO persona(curp,nombre,apellido_paterno,apellido_materno,sexo,fecha_nacimiento,residencia,calle,numero,colonia,cp,municipio,estado,telefono,correo,dependencia,carrera_unach,matricula,password,id_rol,fecha_registro) VALUES(:curp,:nombre,:apellido_paterno,:apellido_materno,:sexo,:fecha_nacimiento,:residencia,:calle,:numero,:colonia,:cp,:municipio,:estado,:telefono,:correo,:dependencia,:carrera_unach,:matricula,:password,:id_rol,:fecha_registro)");
...
$conn->exec('UNLOCK TABLES');
	...
	$conn->rollback();
		...
		protected static function inscribir_alumno_modelo($datos){
			...
			$conn=mainModel::conectar();
				$sql=$conn->prepare("UPDATE persona SET curp=:curp,nombre=:nombre,apellido_paterno=:apellido_paterno,apellido_materno=:apellido_materno,sexo=:sexo,fecha_nacimiento=:fecha_nacimiento,calle=:calle,numero=:numero,colonia=:colonia,cp=:cp,municipio=:municipio,estado=:estado,telefono=:telefono,correo=:correo,dependencia=:dependencia,carrera_unach=:carrera_unach,matricula=:matricula, id_rol=:id_rol WHERE id_persona=:id");
try {
$conn->beginTransaction();
$conn->exec('LOCK TABLES persona, curso_asesorias');
