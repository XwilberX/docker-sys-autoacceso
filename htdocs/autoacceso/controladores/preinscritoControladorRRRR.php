<?php 
	...
	class preinscritoControlador extends preinscritoModelo{
		...
		public function agregar_preinscrito_controlador(){
			...
			if($nombre=="" || $apellido_paterno=="" || $apellido_materno=="" || $curp=="" || $sexo=="" || $tipo=="" || $idioma=="" ||  $fecha_nacimiento=="" || $calle=="" || $numero=="" || $colonia=="" || $cp=="" || $municipio=="" || $estado==""  || $telefono=="" || $correo==""  || $password=="" || $confirmPass=="" ){
...
/*
				...
				*/
					...
					if($idioma_cuatro!=""){
						...
						if($agregar_preinscrito->rowCount()==1){
							...
							"Texto"=>"La solicitud de inscripción se ha guardado correctamente. Su CURP será su usuario de acceso al sistema una vez que se complete el proceso de inscripción.",
							"Tipo"=>"success"
							];
						}else{
							$alerta=[
			...
			public function paginador_preinscrito_controlador($pagina, $registros, $privilegio, $curp, $url, $busqueda){
				...
				if(isset($busqueda) && $busqueda !=""){
                    ...
                    $consulta2="SELECT * FROM persona JOIN curso_asesorias ON persona.id_persona=curso_asesorias.id_persona_alumno JOIN idioma ON curso_asesorias.id_idioma=idioma.id_idioma WHERE persona.curp!='$curp' AND id_rol='7' ORDER BY apellido_paterno ASC";

					$consulta="SELECT * FROM persona WHERE curp!='$curp' AND id_rol='7' ORDER BY apellido_paterno ASC LIMIT $inicio,$registros ";

					$consulta3="SELECT * FROM persona WHERE curp!='$curp' AND id_rol='7' ORDER BY apellido_paterno ASC";
							...
							foreach ($datos as $rows) {
								//if(!in_array($rows['id_persona'], $temporal_Array)){
									...
									<tr class="text-center" >
						                ...
						                <td data-label="Dirección">'.$rows['calle'].' '.$rows['numero'].' '.$rows['colonia'].' Cp: '.$rows['cp'].'</td>
