	...
	class asesoriasControlador extends asesoriasModelo{
		...
		session_start(['name'=>'SCAA']);
				...
				if(isset($_POST['id_asesor_recargar'])){
					$id_repuesto=mainModel::limpiar_cadena($_POST['id_asesor_recargar']);
				}else {
		...
		public function lista_asesorias_controlador($pagina, $registros, $privilegio, $curp, $url, $busqueda){
			...
			if($_SESSION['privilegio_scaa']<=2){
					$consulta="SELECT *,p2.nombre AS nombre2,p2.apellido_paterno AS apPaterno2,p2.apellido_materno AS apMaterno2,p2.correo AS correo2, persona.nombre AS nombre1,persona.apellido_paterno AS apPaterno1,persona.apellido_materno AS apMaterno1,persona.correo AS correo1,persona.telefono AS tel, persona.dependencia AS dependencia FROM persona JOIN curso_asesorias ON persona.id_persona=curso_asesorias.id_persona_alumno JOIN asesorias ON curso_asesorias.id_curso_asesoria=asesorias.id_curso JOIN plantilla_horarios ON asesorias.id_asesoria_plantilla_horario=plantilla_horarios.id_plantilla_horario JOIN persona AS p2 ON p2.id_persona=plantilla_horarios.id_persona_asesor JOIN idioma ON idioma.id_idioma=curso_asesorias.id_idioma WHERE (plantilla_horarios.estatus_hora_asesoria='1' OR plantilla_horarios.estatus_hora_asesoria='3') AND persona.id_rol='6' ORDER BY asesorias.fecha_asesoria, plantilla_horarios.hora ASC";

					$consulta2="SELECT *,p2.nombre AS nombre2,p2.apellido_paterno AS apPaterno2,p2.apellido_materno AS apMaterno2,p2.correo AS correo2, persona.nombre AS nombre1,persona.apellido_paterno AS apPaterno1,persona.apellido_materno AS apMaterno1,persona.correo AS correo1,persona.telefono AS tel, persona.dependencia AS dependencia FROM persona JOIN curso_asesorias ON persona.id_persona=curso_asesorias.id_persona_alumno JOIN asesorias ON curso_asesorias.id_curso_asesoria=asesorias.id_curso JOIN plantilla_horarios ON asesorias.id_asesoria_plantilla_horario=plantilla_horarios.id_plantilla_horario JOIN persona AS p2 ON p2.id_persona=plantilla_horarios.id_persona_asesor JOIN idioma ON idioma.id_idioma=curso_asesorias.id_idioma WHERE (plantilla_horarios.estatus_hora_asesoria='1' OR plantilla_horarios.estatus_hora_asesoria='3') AND persona.id_rol='6' ORDER BY asesorias.fecha_asesoria, plantilla_horarios.hora ASC LIMIT $inicio,$registros";
				...
				$tabla.='						
					...
					if($_SESSION['privilegio_scaa']==3 || $_SESSION['privilegio_scaa']==4){					    
					...
					$tabla.='	</tr >
							...
							foreach ($datos2 as $rows) {
								...
								if($rows['tiempo_hora_asesoria']=="3"){
									$tipoAsesoria="Presencial 30 minutos";
								}
                                if($rows['lc']=="0"){
									$comprension_lectora="NO";
								}
								if($rows['lc']=="1"){
									$comprension_lectora="SÍ";
								}
						...
						<tr class="text-center" >
			                ...
			                $tabla.='<td data-label="Nivel">'.$rows['nivel'].'</td>';
			                 ...
			                 $tabla.='<td data-label="Comprensión lectora">'.$comprension_lectora.'</td>';
								if($_SESSION['privilegio_scaa']==4){
									$tabla.='<td data-label="Dependencia">'.$rows['dependencia'].'</td>';
								}
			                 $tabla.='<td data-label="Teléfono">'.$rows['tel'].'</td>';
							}
							if($_SESSION['privilegio_scaa']<=2){
							    $tabla.='<td data-label="Idioma">'.$rows['etiqueta_idioma'].'</td>';
			...
			public function agregar_horarios_asesor_controlador(){
				...
				if(!isset($_SESSION['id_asesor_cambio'])){
					$id=mainModel::decryption($_POST['user_id_decrypt']);
					$id=mainModel::limpiar_cadena($id);	
				}else{
					$id=$_SESSION['id_asesor_cambio'];
				}
...
// exit();
				...
				$consulta2_1=mainModel::conectar()->prepare("SELECT * FROM  persona  JOIN curso_asesorias ON persona.id_persona=curso_asesorias.id_persona_alumno WHERE persona.id_persona='$id_repuesto' AND curso_asesorias.id_idioma='$id_idioma' AND persona.dependencia='Conversación'");
				$consulta2_1->execute();
				$datos2_1 = $consulta2_1->fetch();
				...
				if($id_idioma==1 && $dato3['tiempo_hora_asesoria']!="2"){
				    ...
				    if ($consulta2_1->rowCount()>=1) {					
						$alerta=[
							"Alerta"=>"simple",
							"Titulo"=>"Ocurrió un error inesperado",
							"Texto"=>"Los estudiantes de departamento solo pueden solicitar club de conversación.",
							"Tipo"=>"error"
						];
						echo json_encode($alerta);
						exit();
		...
		if($id_idioma==1 && $dato4['nivel']>=9){
				...
				if ($dato3['tiempo_hora_asesoria']==1){
					if($consulta13->rowCount()!=0){
						$alerta=[
							"Alerta"=>"simple",
							"Titulo"=>"Ocurrió un error inesperado",
							"Texto"=>"La sesión que intentas agendar choca con otra asesoría que tienes agendadas. Verifica tu lista de asesorías pendientes y prueba con otro horario.",
							"Tipo"=>"error"
						];
						echo json_encode($alerta);
						exit();
				...
				if($datos10['estatus_hora_asesoria']==1){
						$alerta=[
							"Alerta"=>"simple",
							"Titulo"=>"Ocurrió un error inesperado",
							"Texto"=>"Éste horario ya ha sido ocupado por otro alumno, intente con otro horario.",
							"Tipo"=>"error"
						];
						echo json_encode($alerta);
						exit();
 ...
 // $consulta5->debugDumpParams();
  ...
  // print_r($dato4['nivel']);
				...
				if($consulta5_5->rowCount()>=1){
					...
					if($dato4['nivel']<$dato7['nivel']-1 || $dato4['nivel']>$dato7['nivel']+1){
						$alerta=[
							"Alerta"=>"simple",
							"Titulo"=>"Ocurrió un error inesperado",
							"Texto"=>"Los participantes agendados a está sesión tienen un rango nivel en el que usted no tiene permitido participar. Intente con otro club de conversación.",
							"Tipo"=>"error"
						];
						echo json_encode($alerta);
						exit();	
				...
				if($consulta2->rowCount()>=1){
					if($dato3['tiempo_hora_asesoria']==0){
						if($consulta15->rowCount()>=2){
						$alerta=[
							"Alerta"=>"simple",
							"Titulo"=>"Ocurrió un error inesperado",
							"Texto"=>"No puede agendar más de 2 sesines de tutorias de 30 minutos en un solo día.",
							"Tipo"=>"error"
						];
						echo json_encode($alerta);
						exit();
						...
						if($consulta2->rowCount()>=4){

							$alerta=[
								"Alerta"=>"simple",
								"Titulo"=>"Ocurrió un error inesperado",
								"Texto"=>"Usted solo tiene permitido solicitar 4 asesorías por semana.",
								"Tipo"=>"error"
							];
							echo json_encode($alerta);
							exit();
					...
					if($dato3['tiempo_hora_asesoria']==1){
						...
						if($consulta2->rowCount()>=2){
							...
							$alerta=[
								"Alerta"=>"simple",
								"Titulo"=>"Ocurrió un error inesperado",
								"Texto"=>"Usted solo tiene permitido solicitar 2 revisiones de escritos por semana.",
								"Tipo"=>"error"
							];
							echo json_encode($alerta);
							exit();
					...
					if($dato3['tiempo_hora_asesoria']==2){
						...
						if($consulta2->rowCount()>=2){
							...
							$alerta=[
								"Alerta"=>"simple",
								"Titulo"=>"Ocurrió un error inesperado",
								"Texto"=>"Usted solo tiene permitido solicitar 2 conversaciones por semana.",
								"Tipo"=>"error"
							];
							echo json_encode($alerta);
							exit();
...
// $consulta14->debugDumpParams();
     // echo "************";
     // $consulta13->debugDumpParams();
			/*	print_r($datos10);
			*/	
			   // exit();
...
// print($dato3['tiempo_hora_asesoria']);
// exit();
 ...
 // $consulta5->debugDumpParams();
  ...
  // print_r($dato4['nivel']);
				...
				if ($dato4['lc']=="1"){
				 	if ($dato3['tiempo_hora_asesoria']!="0" && $dato3['tiempo_hora_asesoria']!="3"){
				 		$alerta=[
									"Alerta"=>"simple",
									"Titulo"=>"Ocurrió un error inesperado",
									"Texto"=>"Usted solo puede agendar asesorías de 30 minutos.",
				 	...
				 	if($consulta2->rowCount()>=1){
							if($consulta2->rowCount()>=$datos2['tipo_3']){
								...
								$alerta=[
									"Alerta"=>"simple",
									"Titulo"=>"Ocurrió un error inesperado",
									"Texto"=>"Usted solo tiene permitido solicitar ".$datos2['tipo_3']." sesiones de lectura por semana.",
									"Tipo"=>"error"
								];
								echo json_encode($alerta);
								exit();
            ...
            if(isset($_POST['id_idioma_btns_asesorias_maestros'])){
				$consulta= "SELECT * FROM persona JOIN curso_asesorias ON persona.idioma_asesor=curso_asesorias.id_idioma WHERE (persona.id_rol='4' OR persona.id_rol='1' OR persona.id_rol='2') AND curso_asesorias.id_persona_alumno='".$_SESSION['id_scaa']."' AND curso_asesorias.id_idioma='".$_POST['id_idioma_btns_asesorias_maestros']."' ORDER BY persona.nombre, persona.apellido_paterno, persona.apellido_materno";
			}else{
				$consulta= "SELECT * FROM persona JOIN curso_asesorias ON persona.idioma_asesor=curso_asesorias.id_idioma WHERE (persona.id_rol='4' OR persona.id_rol='1' OR persona.id_rol='2') AND curso_asesorias.id_persona_alumno='".$_SESSION['id_scaa']."' AND curso_asesorias.id_idioma='".$_POST['id_idioma_btns_asesorias_maestros']."' ORDER BY persona.nombre, persona.apellido_paterno, persona.apellido_materno";
			}
			...
			$tabla.='
					...
					$tabla.='<form class="FormularioAjax" id="FormularioAjaxAgendar" action="'.SERVERURL.'ajax/asesoriasAjax.php" method="POST" data-form="save">
	        		 			...
	        		 			<input type="hidden" id="id_repuesto" name="id_repuesto" value="'.$_SESSION['id_scaa'].'">
						...
						$tabla.='<form class="FormularioAjax" action="'.SERVERURL.'ajax/asesoriasAjax.php" method="POST" autocomplete="off">';
						foreach ($datos as $rows) {
	 ...
	 // exit();
		...
		public function datos_asesoria_coordinacion_controlador($tipo, $id){
					...
					$tabla.='	</tr >
						...
						<tr class="text-center" >
			                ...
			                $tabla.='<td data-label="Asesor">'.$rows['nombre2'].'</td>';
			                	$tabla.='<td data-label="Avance"><a href="'.SERVERURL.'avanceAsesoria/'.mainModel::encryption($rows['id_asesoria']).'/"><button class="btnActualizar">Avance</button></a>
			                   	</td>';
							}
							if($_SESSION['privilegio_scaa']<=2){
            ...
            public function buscar_proximas_asesorias_controlador($pagina, $registros, $privilegio, $curp, $url, $busqueda){
				$pagina=mainModel::limpiar_cadena($pagina);
				$registros=mainModel::limpiar_cadena($registros);
				$privilegio=mainModel::limpiar_cadena($privilegio);
				$curp=mainModel::limpiar_cadena($curp);
				...
				$inicio= ($pagina>0) ? (($pagina*$registros)-$registros) : 0 ;
					...
					if($busqueda!=""){
						$consulta2="SELECT *,p2.nombre AS nombre2,p2.apellido_paterno AS apPaterno2,p2.apellido_materno AS apMaterno2,p2.correo AS correo2, persona.nombre AS nombre1,persona.apellido_paterno AS apPaterno1,persona.apellido_materno AS apMaterno1,persona.correo AS correo1,persona.telefono AS tel, persona.dependencia AS dependencia FROM persona JOIN curso_asesorias ON persona.id_persona=curso_asesorias.id_persona_alumno JOIN asesorias ON curso_asesorias.id_curso_asesoria=asesorias.id_curso JOIN plantilla_horarios ON asesorias.id_asesoria_plantilla_horario=plantilla_horarios.id_plantilla_horario JOIN persona AS p2 ON p2.id_persona=plantilla_horarios.id_persona_asesor JOIN idioma ON idioma.id_idioma=curso_asesorias.id_idioma WHERE(p2.nombre LIKE '%$busqueda%' OR p2.apellido_paterno LIKE '$busqueda%' OR p2.apellido_materno LIKE '%$busqueda%' OR p2.curp LIKE '%$busqueda%') AND (plantilla_horarios.estatus_hora_asesoria='1' OR plantilla_horarios.estatus_hora_asesoria='3') ORDER BY asesorias.fecha_asesoria ASC, plantilla_horarios.hora ASC LIMIT $inicio,$registros";

						$consulta="SELECT *,p2.nombre AS nombre2,p2.apellido_paterno AS apPaterno2,p2.apellido_materno AS apMaterno2,p2.correo AS correo2, persona.nombre AS nombre1,persona.apellido_paterno AS apPaterno1,persona.apellido_materno AS apMaterno1,persona.correo AS correo1,persona.telefono AS tel, persona.dependencia AS dependencia FROM persona JOIN curso_asesorias ON persona.id_persona=curso_asesorias.id_persona_alumno JOIN asesorias ON curso_asesorias.id_curso_asesoria=asesorias.id_curso JOIN plantilla_horarios ON asesorias.id_asesoria_plantilla_horario=plantilla_horarios.id_plantilla_horario JOIN persona AS p2 ON p2.id_persona=plantilla_horarios.id_persona_asesor JOIN idioma ON idioma.id_idioma=curso_asesorias.id_idioma WHERE(p2.nombre LIKE '%$busqueda%' OR p2.apellido_paterno LIKE '$busqueda%' OR p2.apellido_materno LIKE '%$busqueda%' OR p2.curp LIKE '%$busqueda%') AND (plantilla_horarios.estatus_hora_asesoria='1' OR plantilla_horarios.estatus_hora_asesoria='3') ORDER BY asesorias.fecha_asesoria ASC, plantilla_horarios.hora ASC LIMIT $inicio,$registros";

						$consulta3="SELECT *,p2.nombre AS nombre2,p2.apellido_paterno AS apPaterno2,p2.apellido_materno AS apMaterno2,p2.correo AS correo2, persona.nombre AS nombre1,persona.apellido_paterno AS apPaterno1,persona.apellido_materno AS apMaterno1,persona.correo AS correo1,persona.telefono AS tel, persona.dependencia AS dependencia FROM persona JOIN curso_asesorias ON persona.id_persona=curso_asesorias.id_persona_alumno JOIN asesorias ON curso_asesorias.id_curso_asesoria=asesorias.id_curso JOIN plantilla_horarios ON asesorias.id_asesoria_plantilla_horario=plantilla_horarios.id_plantilla_horario JOIN persona AS p2 ON p2.id_persona=plantilla_horarios.id_persona_asesor JOIN idioma ON idioma.id_idioma=curso_asesorias.id_idioma WHERE(p2.nombre LIKE '%$busqueda%' OR p2.apellido_paterno LIKE '$busqueda%' OR p2.apellido_materno LIKE '%$busqueda%' OR p2.curp LIKE '%$busqueda%') AND (plantilla_horarios.estatus_hora_asesoria='1' OR plantilla_horarios.estatus_hora_asesoria='3')  ORDER BY asesorias.fecha_asesoria ASC, plantilla_horarios.hora ASC";
			...
			public function lista_asesorias_exalumno_controlador($pagina, $registros, $privilegio, $curp, $url, $busqueda){

				$pagina=mainModel::limpiar_cadena($pagina);
				$id_alumno=mainModel::decryption($pagina);
				$registros=mainModel::limpiar_cadena($registros);
				$privilegio=mainModel::limpiar_cadena($privilegio);
				$curp=mainModel::limpiar_cadena($curp);
				...
				if(isset($busqueda) && $busqueda !=""){
					...
					$consulta="SELECT *,p2.nombre AS nombre2,p2.apellido_paterno AS apPaterno2,p2.apellido_materno AS apMaterno2,p2.correo AS correo2, persona.nombre AS nombre1,persona.apellido_paterno AS apPaterno1,persona.apellido_materno AS apMaterno1,persona.correo AS correo1 FROM persona JOIN curso_asesorias ON persona.id_persona=curso_asesorias.id_persona_alumno JOIN asesorias ON curso_asesorias.id_curso_asesoria=asesorias.id_curso JOIN plantilla_horarios ON asesorias.id_asesoria_plantilla_horario=plantilla_horarios.id_plantilla_horario JOIN persona AS p2 ON p2.id_persona=plantilla_horarios.id_persona_asesor JOIN idioma ON idioma.id_idioma=curso_asesorias.id_idioma WHERE plantilla_horarios.estatus_hora_asesoria='2' AND persona.id_rol='8' AND persona.id_persona='$id_alumno' AND asesorias.fecha_asesoria ORDER BY asesorias.fecha_asesoria DESC, plantilla_horarios.hora  DESC";

					$consulta2="SELECT *,p2.nombre AS nombre2,p2.apellido_paterno AS apPaterno2,p2.apellido_materno AS apMaterno2,p2.correo AS correo2, persona.nombre AS nombre1,persona.apellido_paterno AS apPaterno1,persona.apellido_materno AS apMaterno1,persona.correo AS correo1 FROM persona JOIN curso_asesorias ON persona.id_persona=curso_asesorias.id_persona_alumno JOIN asesorias ON curso_asesorias.id_curso_asesoria=asesorias.id_curso JOIN plantilla_horarios ON asesorias.id_asesoria_plantilla_horario=plantilla_horarios.id_plantilla_horario JOIN persona AS p2 ON p2.id_persona=plantilla_horarios.id_persona_asesor JOIN idioma ON idioma.id_idioma=curso_asesorias.id_idioma WHERE plantilla_horarios.estatus_hora_asesoria='2' AND persona.id_rol='8' AND persona.id_persona='$id_alumno' AND asesorias.fecha_asesoria ORDER BY asesorias.fecha_asesoria DESC, plantilla_horarios.hora  DESC LIMIT $inicio,$registros";
				...
				$tabla.='						
					...
					$tabla.='	</tr >
						...
						<tr class="text-center" >
							...
							<td data-label="Condición">'.$rows['condicion'].'</td>
							<td data-label="Asesor">'.$rows['nombre2'].'</td>
							<td data-label="Avance"><a href="'.SERVERURL.'avanceAsesoria/'.mainModel::encryption($rows['id_asesoria']).'/"><button class="btnActualizar">Avance</button></a>
			                   	</td>

			                <td data-label="Completada">';
